...
Local build **0.4.5.0** (WIP: request-aware pools, provider payload adapter, hop `exact_model_only`) installed to `/Applications/llm-swarm-router.app` on Mac mini gateway (`098fceb0`, `10.0.0.32`).
...
uv run pytest tests/test_model_pools.py tests/test_agent_hop_routing.py \
  tests/test_provider_payload.py tests/test_routing_hardening.py -q
```
...
- `packages/netllm-core/src/netllm_core/pool.py` — request-aware overflow
- `packages/netllm-core/src/netllm_core/provider_payload.py` — new
- `packages/netllm-agent/src/netllm_agent/service.py` — adapter + hop wiring
- Tests + `config.example.toml` comment update
