...
- **Cloud settings** (`CloudSettingsView.swift`, Settings → Cloud sidebar row): `document.cloud` (enable/fallback/per-provider enable+region+api_format) round-trips through `netllm config export/import` like every other section; API keys are **not** in `NetllmConfigDocument` — they live in Keychain only (`KeychainStore.Account.{anthropic,openai,moonshot,zai,openrouter}APIKey`), injected into the agent subprocess env by `PythonRuntime.injectCloudAPIKeys` using each provider's `api_key_env` name from the Python registry (`netllm_core.cloud_providers`) — restart the agent to pick up a changed key. `SettingsViewModel.cloudProviders` mirrors the Python registry's display metadata only (base URLs stay server-side).
