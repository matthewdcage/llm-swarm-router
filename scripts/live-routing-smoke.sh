...
status_routed() {
  curl -sf "$BASE/netllm/v1/telemetry?scopes=router" | python3 -c "
import sys, json
...
if out=$(chat "qwen3-next-80b" 2>&1); then
  model_used=$(echo "$out" | python3 -c "import sys,json; print(json.load(sys.stdin).get('model','?'))" 2>/dev/null || echo "?")
...
if [[ "$code" == "200" ]]; then
  model_used=$(python3 -c "import json; print(json.load(open('/tmp/qwen-alias.json')).get('model','?'))" 2>/dev/null || echo "?")
...
wait
qwen_fail=$(rg -l '^fail$' "$tmpdir"/qwen-*.json 2>/dev/null | wc -l | tr -d ' ')
gemma_fail=$(rg -l '^fail$' "$tmpdir"/gemma-*.json 2>/dev/null | wc -l | tr -d ' ')
