...
- **Telemetry + menu stats:** `GET /netllm/v1/telemetry?scopes=router,omlx&watch=1&history=60` unifies router counters (persisted in `~/.config/netllm/stats.json`) with lazy oMLX admin stats/activity probes; web dashboard **Serving** tab + macOS menubar **Serving Stats** / **System Stats** consume it; menubar uses AppKit `MenubarController` with optional gauge NSStatusItems (`ui.menubar_show_*`, `ui.model_favorites`); release notes [docs/release-notes/v0.4.5.0.md](docs/release-notes/v0.4.5.0.md)
