...
- `./netllm` wrapper runs `uv run --directory $ROOT netllm`: no global install needed; `scripts/agent-verify-setup.sh` prefers global `netllm` when on PATH — use `./netllm` for repo-local smoke; for live mesh tuning edit `~/.config/netllm/config.toml` directly and apply via menubar **Restart Agent** (global `netllm` on PATH outside the repo — `./netllm` only exists at repo root)
...
Updated: 2026-07-28 (Serving/telemetry UI contracts; menubar AgentHTTP URL helper)
