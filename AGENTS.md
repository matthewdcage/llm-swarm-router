...
- **Telemetry + menu stats:** `GET /netllm/v1/telemetry?scopes=router,omlx&watch=1&history=60` unifies router counters (persisted in `~/.config/netllm/stats.json`) with lazy oMLX admin probes; web **Serving** tab (router-first: tokens, routed-by-backend, **source + scenario** harness counts, in-flight) + macOS **Serving Stats** submenu (source counts via `ServingStatsMenuBuilder`; menubar HTTP uses `AgentHTTP.url`, not `appendingPathComponent` for query strings; scenario keys dashboard-only) and **System Stats**; optional gauge NSStatusItems (`ui.menubar_show_*`, `ui.model_favorites`); per-model request metrics are not tracked — see Models tab AGENTS note; release notes [docs/release-notes/v0.5.0.0.md](docs/release-notes/v0.5.0.0.md)
- **Main @ `1835c8b` (2026-08-03):** **v0.5.0.0 shipped** (#42 engine refactor + Homebrew #44). Phase B closeout (this branch → **0.5.0.1**) closes F-28 (docs), F-38, F-56, F-57, B5 pool-isolation vector; adds `netllm connect`, scenario Serving UI, Settings/menubar status tests, smoke checklists. Inventory: [docs/closure-roadmap-2026-08-03.md](docs/closure-roadmap-2026-08-03.md).
