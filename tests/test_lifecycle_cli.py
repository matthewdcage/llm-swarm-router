"""Tests for lifecycle command dispatch."""

from __future__ import annotations

from unittest.mock import patch

from netllm_cli.lifecycle import lifecycle_command


def test_lifecycle_source_start_hint() -> None:
    with patch("netllm_cli.lifecycle.get_install_method", return_value="source"):
        rc = lifecycle_command("start")
    assert rc == 1


def test_lifecycle_homebrew_delegates() -> None:
    with patch("netllm_cli.lifecycle.get_install_method", return_value="homebrew"):
        with patch(
            "netllm_cli.lifecycle.darwin.run_brew_services",
            return_value=0,
        ) as brew:
            rc = lifecycle_command("stop")
    assert rc == 0
    brew.assert_called_once_with("stop")


def test_lifecycle_linux_systemd() -> None:
    with patch("netllm_cli.lifecycle.get_install_method", return_value="linux-systemd"):
        with patch(
            "netllm_cli.lifecycle.linux.lifecycle_command",
            return_value=0,
        ) as sysd:
            rc = lifecycle_command("start")
    assert rc == 0
    sysd.assert_called_once_with("start")


def test_lifecycle_windows_service() -> None:
    with patch(
        "netllm_cli.lifecycle.get_install_method", return_value="windows-service"
    ):
        with patch(
            "netllm_cli.lifecycle.windows.lifecycle_command",
            return_value=0,
        ) as win:
            rc = lifecycle_command("restart")
    assert rc == 0
    win.assert_called_once_with("restart")


def test_lifecycle_app_socket_start() -> None:
    with patch("netllm_cli.lifecycle.get_install_method", return_value="app"):
        with patch("netllm_cli.lifecycle.darwin.send_app_control_with_launch") as send:
            send.return_value = {"ok": True, "state": "running", "port": 11400}
            with patch("netllm_cli.lifecycle.darwin.wait_app_control_state") as wait:
                wait.return_value = {"state": "running", "port": 11400}
                rc = lifecycle_command("start", timeout=5.0)
    assert rc == 0
    send.assert_called_once()
