...
- **Web dashboard** (`static/dashboard.js`): status/models load without admin; doctor/config failures degrade gracefully (warn banner, not fatal); cluster token label shows **open (trusted LAN)** when unset; **Discovery tab** `renderDiscoveryCredentialsSection` syncs per-URL API keys into `routing.backends` on Save (same rows as Routing → Backend overrides; macOS Settings mirrors this)
...
- **Serving dashboard tab** (`static/dashboard.js` `renderServingTab`): polls `telemetry?watch=1` plus `/netllm/v1/status` while the tab is visible (5s); router session/all-time tokens and requests always shown; oMLX live/session blocks when `omlx.available`; merges status `source_requests` and `scenario_requests`, telemetry `routed_requests` / `capacity_rejections`, active model + `routing_strategy`. Status tab still shows a subset of routed metrics on load (`watch=0` telemetry snapshot)
