...
- **Overview page** (`static/pages/overview.js`): the former Status and Serving tabs merged. Polls `telemetry?watch=1` plus `/netllm/v1/status` while the page is visible (5s); router session/all-time tokens and requests always shown; oMLX live/session blocks when `omlx.available`; merges status `source_requests` and `scenario_requests`, telemetry `routed_requests` / `capacity_rejections`, active model + `routing_strategy`. **Caveat**: `avg_prefill_tps` / `avg_generation_tps` are not measurements — `service/accounting.py` records `prefill_duration = latency_s * 0.3` and `generation_duration = latency_s * 0.7`, a hardcoded split. Both this page and the macOS menubar display them; see `docs/ui-redesign-feature-spec.md` UI-2
