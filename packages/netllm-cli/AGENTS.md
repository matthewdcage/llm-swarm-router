...
- **Write-path guards:** every CLI/config save that mutates disk calls `netllm_core.config_guards.apply_config_guards` with the same `patch` dict it merged — `config import` passes the full JSON; `sources toggle`, `connect --toggle`, and `ignore add/remove` pass only their section so cloud verification is not re-run on unrelated edits (same contract as the dashboard's section-scoped Save)
